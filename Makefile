...
APPNAME=mtb-example-pmg1-usbpd-drp
...
DEFINES=CY_PD_SINK_ONLY=0 CY_PD_SOURCE_ONLY=0 CY_PD_REV3_ENABLE=1 \
        VBUS_OVP_ENABLE=1 VBUS_UVP_ENABLE=0 VBUS_OCP_ENABLE=1 SYS_DEEPSLEEP_ENABLE=1 \
		 ...
		 CY_APP_POWER_ROLE_PREFERENCE_ENABLE=0 CY_CORROSION_MITIGATION_ENABLE=0

LEGACY_CHARGING_DEFINES=BATTERY_CHARGING_ENABLE=1 LEGACY_PD_PARALLEL_OPER=1 \
		ENABLE_APPLE_BC12_SUPPORT=1 APPLE_SOURCE_DISABLE=0 APPLE_SINK_DISABLE=0 \
		LEGACY_APPLE_SRC_SLN_TERM_ENABLE=1 APPLE_SRC_EXT_TERM_ENABLE=1 
...
ifeq ($(TARGET), APP_PMG1-CY7110)
    DEFINES+=PMG1_V5V_CHANGE_DETECT=0 VBUS_IN_DISCHARGE_EN=1 PMG1_FLIPPED_FET_CTRL=1 CCG_SRC_FET=1 \
             ...
             CY_PD_DEBUG_ACC_DISABLE=1 CY_PD_VDM_DISABLE=1 CCG_TYPE_A_PORT_ENABLE=0
else
    DEFINES+=PMG1_V5V_CHANGE_DETECT=1 CY_PD_CBL_DISC_DISABLE=0 CY_PD_VCONN_DISABLE=0 \
             CY_PD_USB4_SUPPORT_ENABLE=1
endif

ifeq ($(TARGET), APP_PMG1-CY7112)
...
ifneq (,$(filter APP_PMG1-CY7110 APP_PMG1-CY7113 APP_EVAL_PMG1_S3_DUALDRP, $(TARGET)))
    LEGACY_CHARGING_DEFINES+=QC_AFC_CHARGING_DISABLED=0 QC_SRC_AFC_CHARGING_DISABLED=0 QC_AFC_SNK_EN=1
else
    LEGACY_CHARGING_DEFINES+=QC_AFC_CHARGING_DISABLED=1 QC_SRC_AFC_CHARGING_DISABLED=1 QC_AFC_SNK_EN=0
endif

ifneq (,$(filter APP_EVAL_PMG1_S1_DRP APP_EVAL_PMG1_S3_DUALDRP, $(TARGET)))
    DEFINES+=APP_FW_LED_ENABLE=1
else
    DEFINES+=APP_FW_LED_ENABLE=0
endif

DEFINES+=$(LEGACY_CHARGING_DEFINES)
